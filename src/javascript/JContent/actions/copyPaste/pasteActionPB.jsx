...
export const PasteActionComponentPB = withNotifications()(({path, referenceTypes, render: Render, loading: Loading, notificationContext, onAction, onVisibilityChanged, actionData, ...others}) => {
    ...
    const {copyPaste} = useSelector(state => ({
    ...
    const {isVisible, isEnabled, loading, type, possibleReferenceTypes, nodeTypesToSkip, nodes} = useMemo(() => {
        ...
        let isVisible = res.checksResult && res.allowedChildNodeTypes.length > 0 && !childrenLimitReachedOrExceeded(res, templateLimit);
        let isEnabled = true;
        ...
        if (isVisible && isEnabled && nodes.reduce((acc, nodeToPaste) => acc ||
            (type === copyPasteConstants.CUT && nodeToPaste.path === res.path + '/' + nodeToPaste.name) ||
            (isDescendantOrSelf(res.path, nodeToPaste.path)), false)) {
            isEnabled = false;
        }
        ...
        const {loading, checkResult, possibleReferenceTypes} = (isVisible && isEnabled) && nodeTypeCheck(res, nodes, referenceTypes);
...
PasteActionComponentPB.propTypes = {
