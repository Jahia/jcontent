...
export default composeActions(requirementsAction, reduxAction(state => ({language: state.language, availableLanguages: state.jcontent.availableLanguages})), {
    init: context => {
            ...
            _.flatMap(context.availableLanguages, source => _.flatMap(context.availableLanguages, dest => {
                if (source.language !== dest.language && (source.language === context.language || dest.language === context.language) && node['jcr_modifyProperties_default_' + dest.language]) {
                    return [{
                        ...
                        buttonLabel: 'jcontent:label.contentManager.translateAction',
                        buttonLabelParams: {
