...
export const paginationRedux = registry => {
    const paginationReducer = handleActions({
        ...
        '@@router/LOCATION_CHANGE': (state, action) => window.location.pathname === contextJsParameters.contextPath + '/jahia' + action.payload.location.pathname || state.currentPage === 0 ? state : {...state, currentPage: 0}
