...
class CreateAction extends React.Component {
    ...
    render() {
        ...
        // action hvve been processed in Actions.jsx, now only check if there is less than 5 types available
        if (_.size(context.nodeTypes) > Constants.numberOfSplitActionTypes) {
            ...
            return _.map(context.nodeTypes, type => {
                ...
                return (<DxContext.Consumer key={type}>{ dxContext => (<Query query={ContentTypeQuery} variables={{
                    nodeType: type,
